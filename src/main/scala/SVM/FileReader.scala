...
class CSVReader(path: String, separator: Char, columnIndexClass: Int, rowsToCheck: Int = 10) extends FileReader(path) {
  ...
  def read():(DenseMatrix[Double],DenseVector[Int])={
    ...
    for ((line, count) <- bufferedSource.getLines.zipWithIndex; if (count < rowsToCheck)) {
    ...
    for ((line, count) <- bufferedSource2.getLines.zipWithIndex) {
      val array : Array[Double] = line.split(separator).map(_.trim).toArray.map(_.toDouble)
